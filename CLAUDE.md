...
onebot-11-go-sdk 旨在为 Go 语言开发者提供完整、类型安全、易用的 OneBot 11 协议实现。项目采用现代化的 Go 开发实践，通过代码生成保证协议实体的一致性，支持 HTTP 客户端和服务端双向通信。

**核心价值**：

- **完整的协议支持**：覆盖 OneBot 11 全部消息、事件、API 定义
- **类型安全**：基于代码生成的强类型实体，避免运行时错误
- **灵活的通信层**：同时提供 HTTP Client 和 HTTP Server 实现
- **可扩展架构**：基于 Dispatcher/Binder 的插件式处理器注册机制
...
- **语言版本**: Go 1.25
- **核心依赖**: mapstructure (v2.4.0), testify (v1.11.1), yaml.v3
...
graph TD
    ...
    click F "./v11/internal/util/CLAUDE.md" "查看 util 模块文档"
    click H "./v11/cmd/entity-gen/CLAUDE.md" "查看 entity-gen 文档"
    click I "./v11/cmd/bindings-gen/CLAUDE.md" "查看 bindings-gen 文档"
```
...
| 模块路径                  | 职责描述                                | 关键文件                                                 |
| ------------------------- | --------------------------------------- | -------------------------------------------------------- |
| `v11/entity`              | OneBot 11 协议实体定义（消息/事件/API） | `message.go`, `event.go`, `api.go`, `communication.go`  |
| `v11/client`              | HTTP 客户端实现，调用 OneBot API        | `http_client.go`, `http_client_actions.go`              |
| `v11/server`              | HTTP 服务端实现，接收 OneBot 动作请求   | `http_server.go`, `dispatcher.go`, `binder.go`          |
| `v11/internal/util`       | 内部工具函数（JSON 映射、类型转换）     | `util.go`, `util_test.go`                                |
...
- [client 模块](./v11/client/CLAUDE.md) - HTTP 客户端封装
- [server 模块](./v11/server/CLAUDE.md) - HTTP 服务端与分发器
...
| `client`          | `http_client_test.go`           | HTTP 客户端请求构建、错误处理、响应解析       |
| `server`          | `http_server_test.go`, 等       | 路由、鉴权、参数解析、分发器、绑定器          |
| `internal/util`   | `util_test.go`                  | JSON 映射工具的边界条件、性能基准（4 个 Bench）|
...
- **实现 HTTP 服务端**: 参考 `v11/server/CLAUDE.md`
- **添加新 API**:
...
参考 `v11/server/dispatcher.go` 的实现：

```go
dispatcher := server.NewDispatcher()
binder := server.NewBinder("send_private_msg", handleSendPrivateMsg)
