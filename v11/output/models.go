...
type SendPrivateMsgRequest struct {
	...
	// 消息内容是否作为纯文本发送（即不解析 CQ 码），只在 `message` 字段是字符串时有效 | 默认值: false
...
type SendMsgRequest struct {
	// 消息类型，支持 `private`、`group`，分别对应私聊、群组，如不传入，则根据传入的 `*_id` 参数判断 | 默认值: -
	MessageType string `json:"message_type,omitempty"`
	// 对方 QQ 号（消息类型为 `private` 时需要） | 默认值: -
	UserId int64 `json:"user_id,omitempty"`
	// 群号（消息类型为 `group` 时需要） | 默认值: -
	GroupId int64 `json:"group_id,omitempty"`
	// 要发送的内容 | 默认值: -
	// 可以是字符串 (CQ 码格式) 或消息段数组
	Message MessageValue `json:"message,omitempty"`
...
type SetGroupNameResponse struct {
	// 群号 | 默认值: -
	GroupId int64 `json:"group_id,omitempty"`
	// 是否解散，如果登录号是群主，则仅在此项为 true 时能够解散 | 默认值: false
...
type SetGroupLeaveResponse struct {
	...
	// 专属头衔有效期，单位秒，-1 表示永久，不过此项似乎没有效果，可能是只有某些特殊的时间长度有效，有待测试 | 默认值: -1
	Duration int64 `json:"duration,omitempty"`
}
...
type SetFriendAddRequestResponse struct {
	// 加群请求的 flag（需从上报的数据中获得） | 默认值: -
	Flag string `json:"flag,omitempty"`
	// `add` 或 `invite`，请求类型（需要和上报消息中的 `sub_type` 字段相符） | 默认值: -
...
type GetStrangerInfoRequest struct {
	// QQ 号 | 默认值: -
	UserId int64 `json:"user_id,omitempty"`
	// 是否不使用缓存（使用缓存可能更新不及时，但响应更快） | 默认值: false
...
type GetGroupMemberListResponse struct {
	// 群号 | 默认值: -
	GroupId int64 `json:"group_id,omitempty"`
	// 要获取的群荣誉类型，可传入 `talkative` `performer` `legend` `strong_newbie` `emotion` 以分别获取单个类型的群荣誉数据，或传入 `all` 获取所有数据 | 默认值: -
...
type GetRecordRequest struct {
	// 收到的语音文件名（消息段的 `file` 参数），如 `0B38145AA44505000B38145AA4450500.silk` | 默认值: -
	File string `json:"file,omitempty"`
	// 要转换到的格式，目前支持 `mp3`、`amr`、`wma`、`m4a`、`spx`、`ogg`、`wav`、`flac` | 默认值: -
...
type GetImageRequest struct {
	// 收到的图片文件名（消息段的 `file` 参数），如 `6B4DE3DFD1BD271E3297859D41C530F5.jpg` | 默认值: -
	File string `json:"file,omitempty"`
}
...
type SetRestartRequest struct {
	// 要延迟的毫秒数，如果默认情况下无法重启，可以尝试设置延迟为 2000 左右 | 默认值: 0
